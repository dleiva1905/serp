...
$cont_ciclos = 0;
$registros = mysqli_query($mysqli, "SELECT * FROM registro WHERE fecha>='$desde' AND fecha<='$hasta' ".$filtro_operador."");
$cantidad_de_registros = mysqli_num_rows($registros);
$total_recaudado = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT SUM(monto) FROM registro WHERE fecha>='$desde' AND fecha<='$hasta' ".$filtro_operador.""));
...
// Extend the TCPDF class to create custom Header and Footer
    ...
    //Page header
        ...
        // Title
			$this->Cell(0, 15, 'SISTEMA ESPECIALIZADO DE RECAUDACIÓN DE PEAJES - REPORTE DE INGRESOS', 0, false, 'C', 0, '', 0, false, 'M', 'M');
		}
	...
	<table>
		...
		<tr>
			<td height="20" style="text-align:center;background-color: #fdfefe;" colspan="3"><h1>REPORTE DE INGRESOS</h1></td>
		...
		if($operador != '' && $operador != ' '){
			...
			$content .= '
			<tr>
				<th colspan="6" style="text-align:center;"><h3>REPORTE FILTRADO DEL OPERADOR: <u><em>'.$recaudador['nombre'].' '.$recaudador['apellido'].' - C.I: '.number_format($operador, 0, ',', '.').'</em></u><br></h3></th>
			</tr>';
		}
		$content .= '
		<tr>
			<td align="left" style="font-weight: bold">CANTIDAD DE REGISTROS</td>
			<td colspan="2" align="center" style="font-weight: bold">'.number_format($cantidad_de_registros, 0, ',', '.').' Registro(s)</td>
			<td align="left" style="font-weight: bold">     TOTAL RECAUDADO</td>
			<td colspan="2" align="center" style="font-weight: bold">'.number_format($total_recaudado['SUM(monto)'], 0, ',', '.').' Bsf</td>
		</tr>
		<br>
		<tr style="background-color: #f2f3f4;">
			<td align="left" height=" 0" style="font-weight: bold" width="250">TIPO DE VEHÍCULO</td>
			<td align="left" height="20" style="font-weight: bold; width:125px">RECAUDADOR</td>
			<td align="center" height="20" style="font-weight: bold; width:80px">CABINA</td>
			<td align="center" height="20" style="font-weight: bold; width:100px">FECHA</td>
			<td align="center" height="20" style="font-weight: bold; width:150px">HORA</td>
			<td align="center" height="20" style="font-weight: bold; width:80px">VALOR</td>

		</tr>
		';
		while ($registro= mysqli_fetch_assoc($registros)) {
			$tipo = $registro['tipo_de_vehiculo'];

			$tipo_de_vehiculo = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT tipo_de_vehiculo FROM tabulador WHERE id_tabulador='$tipo'"));

			$reca = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT nombre, apellido FROM empleado WHERE cedula='".$registro['recaudador']."'"));
			...
			$content .= '
			<td height="20" align="left">'.$tipo_de_vehiculo['tipo_de_vehiculo'].'</td>
			<td height="20" align="left">'.$reca['nombre'].' '.$reca['apellido'].'</td>
			<td height="20" align="center">'.$registro['cabina'].'</td>
			<td height="20" align="center">'.date("d-m-Y", strtotime($registro['fecha'])).'</td>
			<td height="20" align="center">'.date("h:i A", strtotime($registro['hora'])).'</td>
			<td height="20" align="center">'.number_format($registro['monto'], 0, ',', '.').' Bsf</td>
		</tr>';
...
if($operador != '' && $operador != ' '){
	$pdf->output('Reporte de Ingresos ('.$operador.') '.$desde.' '. $hasta.'.pdf', 'I');
}else{
	$pdf->output('Reporte de Ingresos '.$desde.' '. $hasta.'.pdf', 'I');
}
				...
				<table class="table table-hover">
					...
					<tbody>
						...
						while ($registro=$registros->fetch_assoc()) {
							...
							?>
							<tr>
								<td><?php echo $tipo_de_vehiculo['tipo_de_vehiculo']; ?></td>
								<td><center><?php echo $registro['fecha']; ?></center></td>
								<td><center><?php echo $registro['hora']; ?></center></td>
								<td><center><?php echo number_format($registro['monto'], 0, ',', '.'); ?> Bsf</center></td>
							</tr>
							<?php } ?>
						</tbody>
					</table>
