...
$tabulador = mysqli_query($mysqli, "SELECT * FROM tabulador");

$trafico_total = mysqli_num_rows(mysqli_query($mysqli, "SELECT * FROM registro WHERE fecha>='$desde' AND fecha<='$hasta' ".$filtro_operador.""));
...
// Extend the TCPDF class to create custom Header and Footer
  class MYPDF extends TCPDF {
    ...
    public function Header() {
      ...
      $this->SetFont($font,'',25);
        // Title
      $this->Cell(0, 15, 'SISTEMA ESPECIALIZADO DE RECAUDACIÓN DE PEAJES - REPORTE TOTAL', 0, false, 'C', 0, '', 0, false, 'T', 'M');
    }
  ...
  <table>
    ...
    <tr>
      <td height="20" style="text-align:center;background-color: #fdfefe;" colspan="2"><h1>REPORTE TOTAL</h1></td>
    ...
    <tr style="background-color: #f2f3f4;">
      ...
      <td align="center" height="20" style="font-weight: bold; width:140px">MONTO RECAUDADO</td>
    </tr>
    ';

    while($tipo_de_vehiculo = mysqli_fetch_assoc($tabulador)){
      $tipo = $tipo_de_vehiculo['id_tabulador'];

      $cantidad_por_tipo = mysqli_num_rows(mysqli_query($mysqli, "SELECT * FROM registro WHERE (fecha>='$desde' AND fecha<='$hasta') AND (tipo_de_vehiculo='$tipo') ".$filtro_operador.""));
      ...
      <td height="20" align="center">'.number_format($tipo_de_vehiculo['tarifa'], 0, ',', '.').' Bsf</td>
      <td height="20" align="center">'.number_format($cantidad_por_tipo * $tipo_de_vehiculo['tarifa'], 0, ',', '.').' Bsf</td>
    </tr>
    ';
  }
...
if($operador != '' && $operador != ' '){
  $pdf->output('Reporte Total ('.$operador.') '.$desde.' '. $hasta.'.pdf', 'I');
}else{
  $pdf->output('Reporte Total '.$desde.' '. $hasta.'.pdf', 'I');
}
   ...
   <div class="row">
    <table class="table table-hover">
     ...
     <tbody>
      ...
      while ($tipo=$tabulador->fetch_assoc()) {
        $tipo_actual = $tipo['id_tabulador'];
        $cantidad = mysqli_num_rows(mysqli_query($mysqli, "SELECT * FROM registro WHERE fecha>='$desde' AND fecha<='$hasta' AND tipo_de_vehiculo='$tipo_actual';"));
        ?>
        <tr>
         <td><?php echo $tipo['tipo_de_vehiculo']; ?></td>
         <td><center><?php echo number_format($cantidad, 0, ',', '.'); ?></center></td>
         <td><center><?php echo number_format($tipo['tarifa'], 0, ',', '.');?> Bsf</center></td>
         <td><center><?php echo number_format($cantidad * $tipo['tarifa'], 0, ',', '.'); ?> Bsf</center></td>
       </tr>
       <?php } ?>
       <tr>
         <?php
         ...
         <th><center><?php echo number_format($total_recaudado['SUM(monto)'], 0, ',', '.'); ?> Bsf</center></th>
       </tr>
     </tbody>
   </table>
   <div class="col-md-12">
